...
class DecisionTree:
    ...
    def fit(self, X, y):
        self.n_features = int(np.sqrt(X.shape[1])) if self.n_features is None else self.n_features
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        # Stop growing tree if max depth is reached or too few samples remain
        if depth >= self.max_depth or n_labels == 1 or n_samples < self.min_samples_split:
            return Node(value=self._most_common_label(y))

        # Select best features based on variance
    ...
    def _gini(self, y):
        hist = np.bincount(y)
        ps = hist / len(y)
        return 1 - np.sum(ps ** 2)  # Gini Impurity formula
